...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id EBC2B3C3B5D
	for <lists+linux-usb@lfdr.de>; Sun, 11 Jul 2021 11:54:13 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S232038AbhGKJ4z (ORCPT <rfc822;lists+linux-usb@lfdr.de>);
        Sun, 11 Jul 2021 05:56:55 -0400
Received: from cable.insite.cz ([84.242.75.189]:55908 "EHLO cable.insite.cz"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S229688AbhGKJ4w (ORCPT <rfc822;linux-usb@vger.kernel.org>);
        Sun, 11 Jul 2021 05:56:52 -0400
Received: from localhost (localhost [127.0.0.1])
        by cable.insite.cz (Postfix) with ESMTP id 8F5BBA1A3D403;
        Sun, 11 Jul 2021 11:54:04 +0200 (CEST)
DKIM-Signature: v=1; a=rsa-sha256; c=simple/simple; d=ivitera.com; s=mail;
        t=1625997244; bh=2kDczKluZc6HMBHx4qi1A3f0ESxHRHOdQp+qxeXzzD4=;
        h=From:To:Cc:Subject:Date:In-Reply-To:References:From;
        b=E67i5BnWuSSMwivB8JSWfPZeCKlrrtLqrfp9+Az8LVUXEnU9J0HZy4k/8FS0nKKFQ
         KVX6fl6jK128gLllBxCPnnZUvBQaLyFe0Q6hHxRRqBqKn3fR8IVqYeSvV8Y5xhMomO
         B5m/v25PtpZbGFeurbdddoPLwngW3Q+eYxE3TQDk=
Received: from cable.insite.cz ([84.242.75.189])
        by localhost (server.insite.cz [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id iwhuZgWoVjTM; Sun, 11 Jul 2021 11:53:57 +0200 (CEST)
Received: from precision.doma (ip28.insite.cz [81.0.237.28])
        (Authenticated sender: pavel)
        by cable.insite.cz (Postfix) with ESMTPSA id 16AF4A1A3D405;
        Sun, 11 Jul 2021 11:53:57 +0200 (CEST)
DKIM-Signature: v=1; a=rsa-sha256; c=simple/simple; d=ivitera.com; s=mail;
        t=1625997237; bh=2kDczKluZc6HMBHx4qi1A3f0ESxHRHOdQp+qxeXzzD4=;
        h=From:To:Cc:Subject:Date:In-Reply-To:References:From;
        b=HkUcDBd8di/5obMTWxvAPah2NVKvICma8Phi3WMgQb+lAdMkJP1YSx9MN3IFB7g3U
         +8XotjiG195g6kb+r9N7qMAKTq3OVaKTq5nkupjHZYHZAL7CePyW4VOz32upR51Wv/
         qxzfDEl2W4+qZ+Lmg0P1TB6QGgvsz9cX0fbkfnjk=
...
Subject: [PATCH v2 3/4] usb: gadget: f_uac2: add volume and mute support
Date:   Sun, 11 Jul 2021 11:53:50 +0200
Message-Id: <20210711095351.16877-4-pavel.hofman@ivitera.com>
...
---
 ...
 4 files changed, 641 insertions(+), 63 deletions(-)
...
index ae29ff2b2b68..0e5eef8d5456 100644
...
@@ -774,6 +942,23 @@ afunc_bind(struct usb_configuration *cfg, struct usb_function *fn)
 	us = usb_gstrings_attach(cdev, fn_strings, ARRAY_SIZE(strings_fn));
 	if (IS_ERR(us))
 		return PTR_ERR(us);
+
+	dev_err(dev, "%s:%d FUOUT_EN result: %d!\n", __func__, __LINE__, FUOUT_EN(uac2_opts));
+	dev_err(dev, "%s:%d FUIN_EN result: %d!\n", __func__, __LINE__, FUIN_EN(uac2_opts));
+
...
@@ -787,6 +972,21 @@ afunc_bind(struct usb_configuration *cfg, struct usb_function *fn)
...
@@ -801,6 +1001,26 @@ afunc_bind(struct usb_configuration *cfg, struct usb_function *fn)
...
@@ -808,7 +1028,7 @@ afunc_bind(struct usb_configuration *cfg, struct usb_function *fn)
...
@@ -820,7 +1040,7 @@ afunc_bind(struct usb_configuration *cfg, struct usb_function *fn)
 		ret = usb_interface_id(cfg, fn);
 		if (ret < 0) {
 			dev_err(dev, "%s:%d Error!\n", __func__, __LINE__);
-			return ret;
...
@@ -849,7 +1069,7 @@ afunc_bind(struct usb_configuration *cfg, struct usb_function *fn)
 		ret = usb_interface_id(cfg, fn);
 		if (ret < 0) {
 			dev_err(dev, "%s:%d Error!\n", __func__, __LINE__);
-			return ret;
...
@@ -857,6 +1077,17 @@ afunc_bind(struct usb_configuration *cfg, struct usb_function *fn)
 		uac2->as_in_alt = 0;
 	}
...
+
 	...
 	ret = set_ep_max_packet_size(uac2_opts, &fs_epin_desc, USB_SPEED_FULL,
 				     true);
@@ -904,7 +1135,8 @@ afunc_bind(struct usb_configuration *cfg, struct usb_function *fn)
...
@@ -912,7 +1144,8 @@ afunc_bind(struct usb_configuration *cfg, struct usb_function *fn)
...
@@ -921,7 +1154,8 @@ afunc_bind(struct usb_configuration *cfg, struct usb_function *fn)
...
@@ -937,38 +1171,137 @@ afunc_bind(struct usb_configuration *cfg, struct usb_function *fn)
...
@@ -977,6 +1310,7 @@ afunc_set_alt(struct usb_function *fn, unsigned intf, unsigned alt)
 {
...
+	struct g_audio *agdev = func_to_g_audio(fn);
 	...
 	int ret = 0;
@@ -993,6 +1327,14 @@ afunc_set_alt(struct usb_function *fn, unsigned intf, unsigned alt)
 			dev_err(dev, "%s:%d Error!\n", __func__, __LINE__);
 			return -EINVAL;
 		}
+
...
@@ -1047,6 +1389,8 @@ afunc_disable(struct usb_function *fn)
 	uac2->as_out_alt = 0;
...
+		usb_ep_disable(uac2->int_ep);
 ...
 static int
@@ -1054,7 +1398,7 @@ in_rq_cur(struct usb_function *fn, const struct usb_ctrlrequest *cr)
 {
...
@@ -1063,28 +1407,64 @@ in_rq_cur(struct usb_function *fn, const struct usb_ctrlrequest *cr)
...
+			__func__, __LINE__, entity_id, control_selector);
 	...
 	return value;
@@ -1095,38 +1475,77 @@ in_rq_range(struct usb_function *fn, const struct usb_ctrlrequest *cr)
 {
 	struct usb_request *req = fn->config->cdev->req;
 	struct g_audio *agdev = func_to_g_audio(fn);
-	struct f_uac2_opts *opts;
...
+			__func__, __LINE__, entity_id, control_selector);
 	...
 	return value;
@@ -1143,16 +1562,82 @@ ac_rq_in(struct usb_function *fn, const struct usb_ctrlrequest *cr)
 		return -EOPNOTSUPP;
 }
...
@@ -1228,7 +1713,15 @@ static struct configfs_item_operations f_uac2_item_ops = {
...
+#define UAC2_ATTRIBUTE(type, name)					\
 static ssize_t f_uac2_opts_##name##_show(struct config_item *item,	\
 					 char *page)			\
 {									\
@@ -1236,7 +1729,7 @@ static ssize_t f_uac2_opts_##name##_show(struct config_item *item,	\
 	int result;							\
 									\
...
+	result = sprintf(page, type##_fmt, opts->name);			\
 	mutex_unlock(&opts->lock);					\
 									\
 	return result;							\
@@ -1247,7 +1740,7 @@ static ssize_t f_uac2_opts_##name##_store(struct config_item *item,	\
 {									\
...
+	type num;							\
 	...
 	mutex_lock(&opts->lock);					\
 	if (opts->refcnt) {						\
@@ -1255,7 +1748,7 @@ static ssize_t f_uac2_opts_##name##_store(struct config_item *item,	\
 		goto end;						\
 	}								\
 									\
...
@@ -1345,6 +1850,19 @@ static struct configfs_attribute *f_uac2_attrs[] = {
...
@@ -1383,6 +1901,19 @@ static struct usb_function_instance *afunc_alloc_inst(void)
...
@@ -1409,6 +1940,11 @@ static void afunc_unbind(struct usb_configuration *c, struct usb_function *f)
...
@@ -1441,3 +1977,4 @@ DECLARE_USB_FUNCTION_INIT(uac2, afunc_alloc_inst, afunc_alloc);
 MODULE_LICENSE("GPL");
