Return-Path: <linux-usb+bounces-5726-lists+linux-usb=lfdr.de@vger.kernel.org>
X-Original-To: lists+linux-usb@lfdr.de
Delivered-To: lists+linux-usb@lfdr.de
Received: from am.mirrors.kernel.org (am.mirrors.kernel.org [147.75.80.249])
	by mail.lfdr.de (Postfix) with ESMTPS id 1C93B8455A9
	for <lists+linux-usb@lfdr.de>; Thu,  1 Feb 2024 11:42:20 +0100 (CET)
Received: from smtp.subspace.kernel.org (wormhole.subspace.kernel.org [52.25.139.140])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by am.mirrors.kernel.org (Postfix) with ESMTPS id 1F25B1F23A81
	for <lists+linux-usb@lfdr.de>; Thu,  1 Feb 2024 10:42:19 +0000 (UTC)
Received: from localhost.localdomain (localhost.localdomain [127.0.0.1])
	by smtp.subspace.kernel.org (Postfix) with ESMTP id C9A2C15A490;
	Thu,  1 Feb 2024 10:42:11 +0000 (UTC)
Authentication-Results: smtp.subspace.kernel.org;
	dkim=pass (1024-bit key) header.d=ucw.cz header.i=@ucw.cz header.b="GoEWSO6j"
X-Original-To: linux-usb@vger.kernel.org
Received: from jabberwock.ucw.cz (jabberwock.ucw.cz [46.255.230.98])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by smtp.subspace.kernel.org (Postfix) with ESMTPS id 87BF815B995;
	Thu,  1 Feb 2024 10:42:01 +0000 (UTC)
Authentication-Results: smtp.subspace.kernel.org; arc=none smtp.client-ip=46.255.230.98
ARC-Seal:i=1; a=rsa-sha256; d=subspace.kernel.org; s=arc-20240116;
	t=1706784131; cv=none; b=q8PNgd8Owbf9hO2YKQ7+nyrYIhU2QGH9zfNKlcBeUsiQymjcGRM+eowup6HlI8vFs0jBqkUc4K6Pf08LkCcoZ2FhfXVWSyF7cEBE8N495xHJoJYKe0uNK1yF5G91QoOqsUBTsNpJYk7ftkE6ibCc3zyW4dZnEbC5EHPLOmIskjU=
ARC-Message-Signature:i=1; a=rsa-sha256; d=subspace.kernel.org;
	s=arc-20240116; t=1706784131; c=relaxed/simple;
	bh=ZGSheERQK2Q+qKgYRgGgyuI5rRoT7Nwr80uIGxe3KzY=;
	h=Date:From:To:Subject:Message-ID:MIME-Version:Content-Type:
	 Content-Disposition; b=lyLANAtvcWEIoyPl18OsK8u7RwYVW3KPFyvuFic2U9M9k10J9qfyPWF8dnaWvyzHYT9h7f6vF+LeIZqP43FTK0gDaipioaK91nRm5n6Pk5h0TbHYwWYmg9j4Wr0TIBspxxTpmSbDkvNk51ekzD88omYGP5E/nz9rH+SpGUZ95nw=
ARC-Authentication-Results:i=1; smtp.subspace.kernel.org; dmarc=pass (p=none dis=none) header.from=ucw.cz; spf=pass smtp.mailfrom=ucw.cz; dkim=pass (1024-bit key) header.d=ucw.cz header.i=@ucw.cz header.b=GoEWSO6j; arc=none smtp.client-ip=46.255.230.98
Authentication-Results: smtp.subspace.kernel.org; dmarc=pass (p=none dis=none) header.from=ucw.cz
Authentication-Results: smtp.subspace.kernel.org; spf=pass smtp.mailfrom=ucw.cz
Received: by jabberwock.ucw.cz (Postfix, from userid 1017)
	id C17971C0075; Thu,  1 Feb 2024 11:41:58 +0100 (CET)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=ucw.cz; s=gen1;
	t=1706784118;
	h=from:from:reply-to:subject:subject:date:date:message-id:message-id:
	 to:to:cc:mime-version:mime-version:content-type:content-type:
	 content-transfer-encoding:content-transfer-encoding;
	bh=d8LPd1/75+qHZvBpGeib/lc6S2DuLDFYglPioB8ezjs=;
	b=GoEWSO6jUHeoQv0QMgzfCeTOapnyny6NOcZkMjchghI8sch9/zo4LeoHKKpMjbDdeAhBtp
	2PIgPd5qZrSXgXWr1CoRxReI+bmNC6Dx7/8ptARLyGpbyVt9FSsK4qUmEiiCpWG95hW3PO
	rRyc0CBce0qFI1bdwMp7FH/XbHWefmA=
Date: Thu, 1 Feb 2024 11:41:56 +0100
...
Subject: [PATCH] usb: typec: anx7688: Add driver for ANX7688 USB-C HDMI bridge
Message-ID: <Zbt1dIByBZ2stzjm@mobian>
Precedence: bulk
...
index 000000000000..9b693c9fd085
--- /dev/null
...
+        int ret;
+
+        ret =3D i2c_smbus_write_byte_data(anx7688->client_tcpc, reg_addr, =
value);
+        if (ret < 0)
...
+                cmd, data_len + 3, pkt);
+
+        ret =3D anx7688_tcpc_reg_read(anx7688, ANX7688_TCPC_REG_INTERFACE_=
SEND);
+        if (ret) {
+                dev_err(anx7688->dev,
...
+        for (i =3D 0; i < 300; i++) {
+                ret =3D anx7688_tcpc_reg_read(anx7688, ANX7688_TCPC_REG_IN=
TERFACE_SEND);
+                if (ret <=3D 0)
+                        return ret;
+
...
+
+        ret =3D regulator_enable(anx7688->supplies[ANX7688_VCONN_INDEX].co=
nsumer);
+        if (ret) {
...
+        for (i =3D 0; i < 100; i++) {
+                ret =3D anx7688_reg_read(anx7688, ANX7688_REG_EEPROM_LOAD_=
STATUS0);
+
+                if (ret >=3D 0 && (ret & ANX7688_EEPROM_FW_LOADED) =3D=3D =
...
+        dev_err(anx7688->dev, "boot firmware load failed (you may need to =
...
+		goto err_vconoff;
+
+        ret =3D anx7688_reg_write(anx7688, ANX7688_REG_STATUS_INT_MASK, ~A=
NX7688_SOFT_INT_MASK);
+	if (ret)
+		goto err_vconoff;
+
+        ret =3D anx7688_reg_write(anx7688, ANX7688_REG_IRQ_EXT_SOURCE2, 0x=
ff);
+	if (ret)
+		goto err_vconoff;
+
+        ret =3D anx7688_reg_write(anx7688, ANX7688_REG_IRQ_EXT_MASK2, (u8)=
~ANX7688_IRQ2_SOFT_INT);
+	if (ret)
+		goto err_vconoff;
+
+        /* time to turn off vbus after cc disconnect (unit is 4 ms) */
+        ret =3D anx7688_reg_write(anx7688, ANX7688_REG_VBUS_OFF_DELAY_TIME=
, 100 / 4);
+	if (ret)
+		goto err_vconoff;
+
+        /* 300ms (unit is 2 ms) */
+        ret =3D anx7688_reg_write(anx7688, ANX7688_REG_TRY_UFP_TIMER, 300 =
/ 2);
+	if (ret)
+		goto err_vconoff;
+
+        /* maximum voltage in 100 mV units */
+        ret =3D anx7688_reg_write(anx7688, ANX7688_REG_MAX_VOLTAGE, 50); /=
* 5 V */
+	if (ret)
+		goto err_vconoff;
+
...
+		goto err_vconoff;
+
+        ret =3D anx7688_reg_write(anx7688, ANX7688_REG_MIN_POWER, 1);  /* =
0.5 W */
+	if (ret)
+		goto err_vconoff;
+
...
+};
+
+static const char* anx7688_cmd_statuses[] =3D {
+	"SUCCESS",
+	"REJECT",
+	"FAIL",
+	"BUSY",
...
+					      u8 to_cmd, u8 resp)
+{
+        const char* status =3D resp <=3D CMD_BUSY ? anx7688_cmd_statuses[r=
esp] : "UNKNOWN";
+
+	switch (to_cmd) {
...
+				unsigned voltage =3D pdo_fixed_voltage(pdo);
+				unsigned max_curr =3D pdo_max_current(pdo);
+
+				dev_info(anx7688->dev, "SRC_CAP PDO_FIXED (%umV %umA)\n", voltage, max=
_curr);
+			} else if (pdo_type(pdo) =3D=3D PDO_TYPE_BATT) {
+				unsigned min_volt =3D pdo_min_voltage(pdo);
+				unsigned max_volt =3D pdo_max_voltage(pdo);
+				unsigned max_pow =3D pdo_max_power(pdo);
+
...
+			unsigned rdo_op_curr =3D rdo_op_current(rdo);
+			unsigned rdo_max_curr =3D rdo_max_current(rdo);
+			unsigned rdo_idx =3D rdo_index(rdo) - 1;
...
+                                            ANX7688_TCPC_REG_INTERFACE_REC=
V,
+                                            32, pkt);
+        if (ret < 0) {
...
+        }
+
+        ret =3D anx7688_tcpc_reg_write(anx7688, ANX7688_TCPC_REG_INTERFACE=
_RECV, 0);
+        if (ret) {
...
+        tcpc_status =3D anx7688_tcpc_reg_read(anx7688, ANX7688_TCPC_REG_AL=
ERT0);
+        if (tcpc_status > 0) {
+                anx7688_tcpc_reg_write(anx7688, ANX7688_TCPC_REG_ALERT0, t=
cpc_status);
+        }
+
+        ext2_status =3D anx7688_reg_read(anx7688, ANX7688_REG_IRQ_EXT_SOUR=
CE2);
+        if (ext2_status & ANX7688_IRQ2_SOFT_INT) {
+                soft_status =3D anx7688_reg_read(anx7688, ANX7688_REG_STAT=
US_INT);
...
+		}
+
+                anx7688_reg_write(anx7688, ANX7688_REG_IRQ_EXT_SOURCE2, AN=
X7688_IRQ2_SOFT_INT);
+        }
+
+out_unlock:
...
+}
+
+static int anx7688_eeprom_read(struct anx7688 *anx7688, unsigned addr, u8 =
buf[16])
+{
+        int ret;
+
...
+                return ret;
+
+        ret =3D i2c_smbus_read_i2c_block_data(anx7688->client, 0xd0, 16, b=
uf);
+        if (ret < 0) {
+                dev_err(anx7688->dev,
...
+                ret =3D i2c_smbus_read_i2c_block_data(anx7688->client, add=
r,
+                                                    sizeof buf, buf);
+                if (ret < 0) {
+                        dev_err(anx7688->dev,
...
+{
+	unsigned cc_status =3D anx7688->last_cc_status;
+	union power_supply_propval val =3D {0,};
+        struct device *dev =3D anx7688->dev;
...
+{
+        struct anx7688 *anx7688 =3D container_of(work, struct anx7688, wor=
k.work);
+
...
+
+	ret =3D of_property_read_variable_u32_array(dev->of_node, "source-caps",
+						  anx7688->src_caps,
...
+		return -EPROBE_DEFER;
+
+        anx7688->gpio_enable =3D devm_gpiod_get(dev, "enable", GPIOD_OUT_L=
OW);
...
+        }
+
+        anx7688->gpio_reset =3D devm_gpiod_get(dev, "reset", GPIOD_OUT_HIG=
H);
...
+        }
+
+        anx7688->gpio_cabledet =3D devm_gpiod_get(dev, "cabledet", GPIOD_I=
N);
...
+        anx7688->client_tcpc =3D i2c_new_dummy_device(client->adapter, 0x2=
c);
...
+        anx7688_power_enable(anx7688);
+
+        vid_l =3D anx7688_tcpc_reg_read(anx7688, ANX7688_TCPC_REG_VENDOR_I=
D0);
+        vid_h =3D anx7688_tcpc_reg_read(anx7688, ANX7688_TCPC_REG_VENDOR_I=
D1);
+	if (vid_l < 0 || vid_h < 0) {
...
+	dev_dbg(dev, "enabling USB BC 1.2 detection\n");
+
+        ret =3D devm_request_irq(dev, irq_cabledet, anx7688_irq_plug_handl=
er,
...
+err_disable_reg:
+        regulator_bulk_disable(ANX7688_NUM_ALWAYS_ON_SUPPLIES, anx7688->su=
pplies);
+err_dummy_dev:
...
+
+static struct of_device_id anx7688_of_match_table[] =3D {
