...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id BFC145B271D
	for <lists+linux-usb@lfdr.de>; Thu,  8 Sep 2022 21:48:26 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S229950AbiIHTsK (ORCPT <rfc822;lists+linux-usb@lfdr.de>);
        Thu, 8 Sep 2022 15:48:10 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:55210 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S229800AbiIHTsD (ORCPT
        <rfc822;linux-usb@vger.kernel.org>); Thu, 8 Sep 2022 15:48:03 -0400
Received: from metis.ext.pengutronix.de (metis.ext.pengutronix.de [IPv6:2001:67c:670:201:290:27ff:fe1d:cc33])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id D1F30F4103
        for <linux-usb@vger.kernel.org>; Thu,  8 Sep 2022 12:47:58 -0700 (PDT)
Received: from drehscheibe.grey.stw.pengutronix.de ([2a0a:edc0:0:c01:1d::a2])
        by metis.ext.pengutronix.de with esmtps (TLS1.3:ECDHE_RSA_AES_256_GCM_SHA384:256)
        (Exim 4.92)
        (envelope-from <mgr@pengutronix.de>)
        id 1oWNVE-0001Ml-Ut; Thu, 08 Sep 2022 21:47:57 +0200
Received: from [2a0a:edc0:0:1101:1d::ac] (helo=dude04.red.stw.pengutronix.de)
        by drehscheibe.grey.stw.pengutronix.de with esmtp (Exim 4.94.2)
        (envelope-from <mgr@pengutronix.de>)
        id 1oWNVC-004gYM-C4; Thu, 08 Sep 2022 21:47:56 +0200
Received: from mgr by dude04.red.stw.pengutronix.de with local (Exim 4.94.2)
        (envelope-from <mgr@pengutronix.de>)
        id 1oWNVC-00FjpC-AL; Thu, 08 Sep 2022 21:47:54 +0200
...
Subject: [PATCH v13 6/6] usb: gadget: uvc: add format/frame handling code
Date:   Thu,  8 Sep 2022 21:47:50 +0200
Message-Id: <20220908194750.3750310-7-m.grzeschik@pengutronix.de>
...
The userspace application only needs to be extended with subscription
for streamon, streamoff, connect and disconnect for stream handling and
become able to serve the uvc device.

If the userspace still wants to subscribe for the SETUP and DATA
events, it still can. This way the gadget is not braking current
applications and stays extendable.
...
index 6f1138e819bdbf..f7d538cc1beb5e 100644
...
index d32f34a7dbc423..ad89d969f869c5 100644
...
@@ -189,5 +199,14 @@ extern struct uvcg_format *find_format_by_index(struct uvc_device *uvc,
...
index bc5e2076c6b7e4..d96c7f340839ed 100644
...
@@ -234,6 +262,8 @@ uvc_v4l2_get_format(struct file *file, void *fh, struct v4l2_format *fmt)
...
@@ -259,6 +289,7 @@ uvc_v4l2_try_format(struct file *file, void *fh, struct v4l2_format *fmt)
 	struct uvc_video *video = &uvc->video;
 	struct uvcg_format *uformat;
 	struct uvcg_frame *uframe;
+	int iformat;
...
@@ -274,6 +305,10 @@ uvc_v4l2_try_format(struct file *file, void *fh, struct v4l2_format *fmt)
 	if (!uformat)
 		return -EINVAL;
...
+
 	...
 	if (!uframe)
@@ -320,8 +355,12 @@ uvc_v4l2_enum_frameintervals(struct file *file, void *fh,
 		if (fival->index >= 1)
 			return -EINVAL;
...
+		spin_unlock(&video->frame_lock);
 		...
 		if (fival->index >= uframe->frame.b_frame_interval_type)
 			return -EINVAL;
@@ -353,8 +392,12 @@ uvc_v4l2_enum_framesizes(struct file *file, void *fh,
 		if (fsize->index >= 1)
 			return -EINVAL;
...
+		spin_unlock(&video->frame_lock);
 		...
 		if (!uformat)
@@ -388,7 +431,11 @@ uvc_v4l2_enum_format(struct file *file, void *fh, struct v4l2_fmtdesc *f)
 		if (f->index >= 1)
 			return -EINVAL;
...
+		spin_unlock(&video->frame_lock);
 		...
 		if (f->index >= uvc->header->num_fmt)
 			return -EINVAL;
@@ -512,14 +559,20 @@ uvc_v4l2_subscribe_event(struct v4l2_fh *fh,
...
@@ -549,7 +602,10 @@ uvc_v4l2_unsubscribe_event(struct v4l2_fh *fh,
 	if (ret < 0)
 		return ret;
